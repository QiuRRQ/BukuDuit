...
func (repository TransactionRepository) Read(ID string) (data models.Transactions, err error) {
	statement := `select id, shop_id, reference_id, category_id, amount, description, image, type, transaction_date,
	created_at, updated_at, deleted_at, status, customer_id, books_debt_id, books_transactions_id from "transactions" where "id"=$1 and "deleted_at" is null`
